...
/*	if($cr == 'Contra a pessoa') { $id_cr = 1; }
	...
	elseif($cr == 'Outros criminais (não inclui contravenções)') { $id_cr = 6; }
	elseif($cr == 'Total de Crimes Violentos ( Hom.Doloso, Roubo, Latrocínio, Estupro e EMS)') { $id_cr = 7; }
	elseif($cr == 'Total de delitos') { $id_cr = 8; }
	elseif($cr == 'Não Criminais') { $id_cr = 9; }
	elseif($cr == 'Total de boletins de ocorrência') { $id_cr = 10; }
	elseif($cr == 'Total de termos circunstanciados lavrados pela Polícia Civil') { $id_cr = 11; }
	elseif($cr == 'Total de termos circunstanciados lavrados pela Polícia Militar') { $id_cr = 12; }
	elseif($cr == 'Total de inquéritos instaurados') { $id_cr = 13; }
	elseif($cr == 'Pessoas mortas em confronto com a polícia civil em serviço') { $id_cr = 14; }
	elseif($cr == 'Pessoas mortas por policiais civis de folga') { $id_cr = 15; }
	elseif($cr == 'Pessoas feridas em confronto com a polícia civil em serviço') { $id_cr = 16; }
	elseif($cr == 'Pessoas feridas por policiais civis de folga') { $id_cr = 17; }
	elseif($cr == 'Policiais civis mortos em serviço') { $id_cr = 18; }
	elseif($cr == 'Policiais civis feridos em serviço') { $id_cr = 19; }
	elseif($cr == 'Prisões efetuadas (em flagrante+ por mandado)') { $id_cr = 20; }
	elseif($cr == 'Nº de pessoas presas em flagrante') { $id_cr = 21; }
	elseif($cr == 'Nº de pessoas presas por mandado') { $id_cr = 22; }
	elseif($cr == 'Nº de autos de apreensão (art 173 ECA)') { $id_cr = 23; }
	elseif($cr == 'Nº de infratores apreendidos em flagrante') { $id_cr = 24; }
	elseif($cr == 'Nº de infratores apreendidos por mandado') { $id_cr = 25; }
	elseif($cr == 'Nº de revistas pessoais/ identelseificação') { $id_cr = 26; }
	elseif($cr == 'Armas de fogo apreendidas') { $id_cr = 27; }
	elseif($cr == 'Nº de Veículos Recuperados') { $id_cr = 28; }
	elseif($cr == 'Exames Necroscópicos Realizados - IML') { $id_cr = 29; }
	elseif($cr == 'Laudos Necroscópicos Expedidos - IML') { $id_cr = 30; }
	elseif($cr == 'Exames Clínicos Laboratoriais Realizados - IML') { $id_cr = 31; }
	elseif($cr == 'Laudos Clínicos Laboratoriais Expedidos - IML') { $id_cr = 32; }
	elseif($cr == 'Exames Periciais Realizados - IC') { $id_cr = 33; }
	elseif($cr == 'Laudos Periciais Expedidos - IC') { $id_cr = 34; }
	elseif($cr == 'Pessoas mortas em confronto com a polícia militar em serviço') { $id_cr = 35; }
	elseif($cr == 'Pessoas mortas por policiais militares de folga') { $id_cr = 36; }
	elseif($cr == 'Pessoas feridas em confronto com a polícia militar em serviço') { $id_cr = 37; }
	elseif($cr == 'Pessoas feridas por policiais militares de folga') { $id_cr = 38; }
	elseif($cr == 'Policiais militares mortos em serviço') { $id_cr = 39; }
	elseif($cr == 'Policiais militares feridos em serviço') { $id_cr = 40; }
	elseif($cr == 'Homicídio doloso') { $id_cr = 41; }
	elseif($cr == 'Nº de Vítimas em Homicídio Doloso') { $id_cr = 42; }
	elseif($cr == 'Homicídio culposo por Acidente de Trânsito') { $id_cr = 43; }
	elseif($cr == 'Homicídio Culposo') { $id_cr = 44; }
	elseif($cr == 'Tentativa de homicídio') { $id_cr = 45; }
	elseif($cr == 'Lesão corporal dolosa') { $id_cr = 46; }
	elseif($cr == 'Lesão corporal culposa por Acidente de Trânsito') { $id_cr = 47; }
	elseif($cr == 'Lesão corporal culposa outras') { $id_cr = 48; }
	elseif($cr == 'Latrocínio') { $id_cr = 49; }
	elseif($cr == 'Nº de Vítimas de Latrocínio') { $id_cr = 50; }
	elseif($cr == 'Estupro') { $id_cr = 51; }
	elseif($cr == 'Extorsão mediante seqüestro') { $id_cr = 52; }
	elseif($cr == 'Tráfico de entorpecentes') { $id_cr = 53; }
	elseif($cr == 'Roubo - outros') { $id_cr = 54; }
	elseif($cr == 'Roubo de veículos') { $id_cr = 55; }
	elseif($cr == 'Roubo a Banco') { $id_cr = 56; }
	elseif($cr == 'Roubo de Carga') { $id_cr = 57; }
	elseif($cr == 'Furto - outros') { $id_cr = 58; }
	elseif($cr == 'Furto de veículos') { $id_cr = 59; }
*/
...
echo '
<h5>Estatísticas no Estado de São Paulo</h5>
...
echo '<strong>Evolução anual:</strong><br>';
...
YoY('1', $cr, 7, 8);
echo '<br>';
echo '<strong>Ocorrências (por ano):</strong><br>';
Grafico("Capital", $chart_title, '1', $cr);
echo '<br><span><strong>Sazonalidade (por trimestre)</strong></span><br>';
AoLongoDoAno('Ao Longo do Ano', 'Estatística por trimestre', '1', $cr);

echo '<h3>Dados do Estado</h3>';
echo '<strong>Proporção de registros, por região</strong>';
GraficoPizza($cr);

echo '<br><strong>Ocorrências no Estado de São Paulo</strong><br>';
Grafico("Estado", $chart_title, '13', $cr);

echo '<br><strong>Ocorrências no Interior de São Paulo</strong>'.'<br>';
